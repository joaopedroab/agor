...
- **Private DM text only.** Group chats, supergroups, channels, attachments, rich media, and Mini Apps are rejected.
...
- **Connector Layer** — Platform-agnostic `GatewayConnector` interface with a registry pattern. Currently: `SlackConnector` (Socket Mode), `GitHubConnector` (API polling), `TeamsConnector` (Bot Framework webhook), and `TelegramConnector` (opt-in `getUpdates` polling for private text DMs plus rich-markdown replies with plain-text fallback to mapped private DM sessions). Adding a new platform means implementing `sendMessage()` and optionally `startListening()`.
