...
5. **Route response back when supported.** When the agent produces a response, Agor's outbound routing hook can post it back into the same platform thread for outbound-capable connectors. Formatting is adapted per platform (Slack mrkdwn conversion, GitHub markdown pass-through). Telegram supports text-only request/reply outbound for mapped private DM sessions; proactive sends, groups/channels, attachments, rich markdown formatting, and self-service `/link` remain later work.
...
- **Inbound only.** Telegram DMs can create or continue Agor sessions, but Agor does not send outbound or proactive Telegram replies. Agent progress and responses remain visible in Agor.
...
- **Connector Layer** — Platform-agnostic `GatewayConnector` interface with a registry pattern. Currently: `SlackConnector` (Socket Mode), `GitHubConnector` (API polling), `TeamsConnector` (Bot Framework webhook), and `TelegramConnector` (opt-in `getUpdates` polling for private text DMs plus text-only replies to mapped private DM sessions). Adding a new platform means implementing `sendMessage()` and optionally `startListening()`.
