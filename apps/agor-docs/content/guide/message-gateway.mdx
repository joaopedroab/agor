...
A Message Gateway channel opens an external pathway (Slack DM, GitHub `@mention`, Teams chat, or opt-in Telegram DM polling) that can prompt agents holding your branch's full surface area: code, MCP servers, env vars, and any credentials in scope. Worth thinking through before you flip it on, but not categorically dangerous.

Things working in your favor:

- **Explicit user matching.** Inbound Slack messages are aligned to Agor users via the `users:read.email` scope; GitHub mentions are aligned via `user_map` (login → email) or the sender's public GitHub email; Telegram DMs are aligned only through an explicit link to the sender's stable numeric Telegram `user.id`. When a match exists, the spawned session is attributed to that real Agor user — so prompts run under their identity and audit trail, not as a generic bot. Unmatched senders are rejected (or surfaced to admins).
- **Encrypted credentials at rest.** Bot tokens, app tokens, and private keys are stored encrypted in the daemon database.
- **Narrow transports.** Slack uses Socket Mode, GitHub uses API polling, Teams uses the Bot Framework webhook, and Telegram uses polling only when you explicitly opt in with a bot token and `enable_polling`.
...
- **Channels open external pathways into agent context.** Anyone who can reach the configured bot surface (DM, chat, or watched repo mention) can ask the agent to read, write, or execute code on the target branch. If the channel runs in `trust` mode with broad MCP access, that pathway is effectively a remote prompt seat — treat it as such.
- **Restrict who can reach the bot.** Limit Slack DM scope (private workspace or app distribution settings), narrow GitHub `watch_repos` to repos where mentions are intended, and keep Telegram polling disabled unless the bot token and explicit user links are ready.
...
> Currently supports **Slack**, **GitHub**, **Microsoft Teams**, and a narrow **Telegram private-DM MVP**. Discord and WhatsApp connectors are planned.
...
- **"Optimus Prime"** — Claude Code (Opus 4.8) on your main teammate branch, with Slack + GitHub MCP servers
...
Gateway channels keep an internal per-channel key for Agor's generic inbound endpoint, but supported integrations usually do not require you to copy it anywhere. Slack uses Socket Mode, GitHub uses API polling, Teams uses the Bot Framework webhook, and Telegram uses opt-in Bot API polling for private DMs only.
...
5. **Route response back when supported.** When the agent produces a response, Agor's outbound routing hook can post it back into the same platform thread for outbound-capable connectors. Formatting is adapted per platform (Slack mrkdwn conversion, GitHub markdown pass-through). Telegram supports request/reply outbound for mapped private DM sessions with Telegram-safe rich markdown formatting and plain-text fallback; proactive sends, groups/channels, and provider-side setup remain later work.
...
## Telegram Private-DM MVP

Telegram is available as a native Message Gateway channel for a narrow, safe MVP. It is disabled/no-op unless an admin creates an enabled `telegram` channel with a `bot_token` and explicitly sets `config.enable_polling: true`. The Settings UI exposes minimal local fields for the bot token, polling opt-in, transport kill switch, and poll interval; these fields do not configure webhooks, call BotFather, or mutate Telegram provider settings. The connector uses Telegram Bot API `getUpdates` polling only.

Current capabilities and limits:

- **Private DMs only.** Group chats, supergroups, channels, unsupported rich media, and Mini Apps are rejected.
- **Explicit numeric user link only.** A Telegram sender must be linked to an Agor user by the stable numeric Telegram `user.id`. For admin/test setup, create this local link directly; for self-service setup, create a short-lived local token with the `agor_users_telegram_link_token_create` MCP tool, then have the user send `/link <token>` to the bot in a private DM. Telegram usernames, first names, and last names are display metadata only and are never trusted for identity.
- **No owner fallback.** If a Telegram sender is missing, unlinked, or ambiguously linked to more than one Agor user, the gateway rejects the message instead of running it as the channel owner.
- **Local `/link` flow, no provider mutation.** `/link` without a token replies with safe setup help. `/link <token>` verifies a hashed, single-use, TTL-bound local token, links the numeric Telegram `user.id` to the intended Agor user, and consumes the token. Invalid, expired, reused, duplicate, or ambiguous links fail closed and do not create or continue an agent session.
- **Private-DM session lifecycle commands.** `/help` lists the supported Telegram commands. After linking, regular messages continue the currently mapped Agor session. Send `/new` to clear that DM's current session mapping so the next regular message starts a fresh Agor session, or `/new <prompt>` to clear the old mapping and immediately start a fresh session with `<prompt>` as the first prompt. `/new` requires the explicit numeric Telegram link; unlinked senders cannot use it to create sessions.
- **Mapped DM replies with safe formatting.** Telegram DMs can create or continue Agor sessions after linking, and agent replies to those mapped private-DM sessions are sent back with Telegram-safe rich markdown basics (bold, italic, links, inline code, fenced code, and list-like text). If Telegram rejects a rich payload, Agor retries once as plain text. Proactive sends, groups/channels, webhooks, and setup wizard UI remain later work.
- **Inbound attachment subset.** Linked private-DM users can send Telegram documents and photos. Agor downloads them via Telegram `getFile` + file download behind the connector seam, enforces the normal Agor upload allowlist and 50 MB per-file limit, sanitizes filenames, stores bytes under Agor-owned `~/.agor/uploads/`, and prompts the session with local file paths plus untrusted metadata/caption context. Audio, voice, video, video notes, SVG, executables, and other non-allowlisted MIME types fail closed with a safe Telegram reply and do not create or continue a session.
- **Normal Agor authority model.** Gateway-created sessions still use the channel's target branch, permission configuration, sandboxing, MCP attachments, and approval gates. Plain Telegram replies such as “yes” are not approvals for destructive tool calls or provider mutations.
...
- **Connector Layer** — Platform-agnostic `GatewayConnector` interface with a registry pattern. Currently: `SlackConnector` (Socket Mode), `GitHubConnector` (API polling), `TeamsConnector` (Bot Framework webhook), and `TelegramConnector` (opt-in `getUpdates` polling for private DMs, document/photo attachment download seam, plus rich-markdown replies with plain-text fallback to mapped private DM sessions). Adding a new platform means implementing `sendMessage()` and optionally `startListening()`.
