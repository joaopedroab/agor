...
- **Explicit numeric user link only.** A Telegram sender must already be linked to an Agor user by the stable numeric Telegram `user.id`. Telegram usernames, first names, and last names are display metadata only and are never trusted for identity.
