...
> Currently supports **Slack**, **GitHub**, and **Microsoft Teams**. Telegram is planned as a native Gateway Channel; Discord and WhatsApp connectors are also planned.
...
Telegram is being prepared as a native Message Gateway channel, but it does not
have live transport yet. There is no Telegram polling loop, webhook handler, or
send/receive connector in the current release.

The planned MVP is intentionally narrow and safe by default:

- **Private DM text only.** Group chats, channels, attachments, and rich media are outside the MVP.
- **Explicit user linking.** A Telegram sender must be linked to an Agor user by the stable numeric Telegram `user.id`. Telegram usernames are display metadata only and are not trusted for identity.
- **No owner fallback for aligned senders.** If a Telegram sender is not linked, the gateway should reject the message rather than running it as the channel owner.
- **Normal Agor authority model.** Gateway-created sessions still use the channel's target branch, permission configuration, sandboxing, MCP attachments, and approval gates.
- **Approvals stay in Agor.** Plain Telegram replies such as “yes” are not approvals for destructive tool calls or provider mutations.

You may see `telegram` in low-level channel type schemas while this foundation is
in place. Treat it as planned/non-listening until the connector setup is
documented here.
...
- **Connector Layer** — Platform-agnostic `GatewayConnector` interface with a registry pattern. Currently: `SlackConnector` (Socket Mode), `GitHubConnector` (API polling), and `TeamsConnector` (Bot Framework webhook). Telegram is reserved as a planned native channel type but has no registered connector yet. Adding a new platform means implementing `sendMessage()` and optionally `startListening()`.
