...
- **Restrict who can reach the bot.** Limit Slack DM scope (private workspace or app distribution settings), narrow GitHub `watch_repos`, use Shortcut's search scope to constrain which stories can summon the assistant, and keep Telegram polling disabled unless the bot token and explicit user links are ready.
...
> Currently supports **Slack**, **GitHub**, **Shortcut**, **Microsoft Teams**, and a narrow **Telegram private-DM MVP**. Discord and WhatsApp connectors are planned.
