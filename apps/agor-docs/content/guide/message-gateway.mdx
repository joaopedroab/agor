...
- **No self-service setup flow yet.** There is no Telegram setup wizard, webhook handler, or provider-side setup automation in this MVP. `/link` commands are reserved for future explicit-link flows and do not create sessions or mutate links today.
- **Mapped DM replies with safe formatting.** Telegram DMs can create or continue Agor sessions, and agent replies to those mapped private-DM sessions are sent back with Telegram-safe rich markdown basics (bold, italic, links, inline code, fenced code, and list-like text). If Telegram rejects a rich payload, Agor retries once as plain text. Proactive sends, attachments, groups/channels, and self-service `/link` remain later work.
