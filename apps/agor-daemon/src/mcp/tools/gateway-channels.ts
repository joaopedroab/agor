...
function gatewayChannelNextSteps(channel: GatewayChannel): string[] {
  ...
  return [
    ...
    'Telegram supports rich markdown replies with safe plain-text fallback for existing mapped private DM sessions; proactive emits, groups, attachments, self-service /link, Mini Apps, setup wizard, webhooks, and provider mutation remain later work.',
  ];
}
...
export function registerGatewayChannelTools(server: McpServer, ctx: McpContext): void {
  ...
  server.registerTool(
    ...
    {
      description:
        'Create a gateway channel definition (admin-only) through the same gateway-channels service used by the UI. Current connectors: Slack, GitHub, Teams, and Telegram private-DM MVP. Slack example config: { bot_token, app_token, connection_mode:"socket", enable_channels:true, require_mention:true, allowed_channel_ids:["C123"] }. Telegram example config: { bot_token, enable_polling:true } and remains no-op unless enabled with explicit polling; it accepts private DM text only, requires an existing numeric user.id link, has no owner fallback, and supports rich markdown replies with safe plain-text fallback for existing mapped private DM sessions; proactive emits, groups, attachments, self-service /link token flow, and provider mutation remain unsupported/out of scope. Secrets are encrypted by the service and returned redacted; prefer environment/template references where possible because raw secrets in tool arguments may appear in the MCP transcript.',
